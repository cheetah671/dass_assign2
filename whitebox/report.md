...
### Structure Update Note

- Project structure was updated so `moneypoly` now resides under `whitebox/`.
- Whitebox tests should be run from the `whitebox` directory to resolve imports correctly.
...
- `whitebox/tests/test_game_decision_paths.py`
- `whitebox/tests/test_integration_decision_paths.py`
- `whitebox/tests/test_integration_extra.py`

- Coverage intent for this expansion:
- Increase decision-path coverage in game/menu flows.
- Add integration-phase behavior checks across `Game`, `Bank`, `Player`, `Board`, and card actions.
- Validate monetary transfer correctness and cross-module state transitions.
...
- Result: `213 total` (`208 passed`, `5 failed`).

- After additional decision-path tests:
...
- Result: `238 tests collected`, `235 passed`, `3 failed`.


### Note On Test-Setup Correction

- `test_handle_jail_turn_pays_fine_path_resets_jail` initially failed due to confirm-sequence setup, not product logic.
- Test input mocking was corrected; failure removed in subsequent run.


### Failed Test Cases Found (After Additional Coverage)

#### Error 11: Emergency loan does not reduce bank reserves

- Failing test:
- `whitebox/tests/test_integration.py::test_integration_bank_loan_updates_player_and_bank_balance`

- Why this test is needed:
- Loan issuance is a money-transfer branch and should update both player and bank state.

- Issue observed:
- `Bank.give_loan()` increases player balance but bank balance remains unchanged.


#### Error 12: Rent payment does not credit property owner

- Failing test:
- `whitebox/tests/test_integration.py::test_integration_purchase_then_rent_transfers_to_owner`

- Why this test is needed:
- Rent is a core gameplay transfer path; debit and credit sides must both be consistent.

- Issue observed:
- Tenant balance decreases, but owner balance does not increase.


#### Error 13: Trade cash transfer does not credit seller

- Failing test:
- `whitebox/tests/test_integration.py::test_integration_trade_transfers_cash_and_property`

- Why this test is needed:
- Trade logic must transfer ownership and cash atomically and consistently.
